...
struct is31fl3743_data {
	...
	/* LED config reg state, IS31FL3743 conf reg is write only */
	uint8_t conf_reg;
};
...
static int is31fl3743_init(const struct device *dev)
{
	...
	data->conf_reg = CONF_REG_SSD_MASK | (config->current_sources << CONF_REG_SWS_SHIFT);
...
int is31fl3743_blank(const struct device *dev, bool blank_en)
{
	...
	return i2c_reg_update_byte_dt(&config->bus, CONF_REG, CONF_REG_SSD_MASK, blank_en);
}
