...
#define HW_ROW_COUNT 9
#define HW_COL_COUNT 39
...
const struct device *led_dev = DEVICE_DT_GET_ONE(issi_is31fl3741);
...
int main(void)
{
	...
	while (1) {
		...
		if (current_limit == 0xFF) {
			...
			current_limit = 0x3F;
			ret = is31fl3741_current_limit(led_dev, current_limit);
			if (ret) {
